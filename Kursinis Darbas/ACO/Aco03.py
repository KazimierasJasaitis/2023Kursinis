...
def pheromone_callback_with_iteration(pheromones, best_solution):
    ...
    if pheromone_callback_with_iteration.iteration % n == 0:
        ...
        plot_solution(cities, best_solution[0])  # Plot the best route here
...
if __name__ == "__main__":
    ...
    aco = AntColonyOptimizer(
        num_ants, alpha, beta, rho, q, pheromone_callback=pheromone_callback_with_iteration
    )
    ...
    aco = AntColonyOptimizer(num_ants, alpha, beta, rho, q)
    best_tour, best_tour_length = aco.optimize(cities)
